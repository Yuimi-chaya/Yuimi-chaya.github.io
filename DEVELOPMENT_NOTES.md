...
- Kisara rejected title-vein experiment checkpoint: `4f39899 Checkpoint Kisara title contract experiment`; reverted by `ef5a991 Revert "Checkpoint Kisara title contract experiment"` before the active material-infusion pass.
- Deployment: Astro static output to GitHub Pages through `.github/workflows/deploy.yml`.
- Local state: the Kisara image transition, title-clipped scarlet material infusion, automatic title-core enchantment prelude, post-gloss procedural warning, segmented foreground pink-black-hole release, side-only WebGL2 background lensing, independent title refraction/dissolve/reconstruction, full-screen block reconstruction, multi-wave aftermath, lingering particle rain, persistent glitch status, post-reconstruction highlights/parallax, and site-wide deep-indigo UI are implemented and browser-validated. The old local charge beam, exterior coils, U-turn flight, and jet/coast/drift wake are disabled and visually absent; the hidden `1px` Canvas remains only as the established animation scheduler. The title keeps its clear red core and soft tide, while the blade gloss remains the Beijing-time July 19 morning slanted text layer clipped inside the glyphs. The black-hole center uses non-rotating Cartesian plasma turbulence, fixed-anchor short inward streams, stationary low-frequency contours, and offset irregular dark layers instead of angular spiral arms, a rotating oval, or a standalone sphere. After reconstruction completes, the existing WebGL title lens now takes full ownership of the `Kisara` glyphs: the CSS source text fades out monotonically while the shader performs continuous liquid UV displacement, broad direction-change pressure deformation, moving pink/blue energy traces, chromatic edge separation, and broken sampled edge drag. The rejected `ba82ee4` 2D overlay and `4f39899` title-vein experiment remain rollback references only; `1725af6` remains the segmented-flight rollback point. Untracked user source materials remain separate and untouched (`102000325_p0.jpg`, `122472458_p0.png`, `gate-background.jpg`, `XP/`, `kisara/`, `showcase-output/`).
...
- Implementation state: three independent frontends are locally available. The Kisara interaction and full-page presentation pass have passed the current local test/build/browser checks and are awaiting final visual acceptance; they have not been pushed. The subject-aware lensing checkpoint is `1164fb4`, the eight-second timing checkpoint is `f20ab43`, and the latest warning rollback point is `d112f11`. The final reconstruction now hands the title directly into the WebGL liquid material instead of restoring an unprocessed red CSS title between effects; this refinement is implemented and locally verified but has not been pushed.
...
- Status: image transition, title-clipped scarlet material infusion, two-stage overcharge sequence, persistent release particle pools, segmented pink-black-hole release, side-field WebGL refraction, post-reconstruction eye/blade highlights, mouse parallax, and the WebGL liquid-title takeover are implemented and browser-validated on desktop and `375x844` mobile viewports; publication is pending.
- Key behavior: while the homepage is at the top, downward wheel/touch/keyboard input raises a blue-to-red diagonal tide and upward input lowers it. Input changes a target level; `requestAnimationFrame` advances the visible level with damped velocity, so the mask, background transition, crest, and title material continue smoothly between wheel events. The cold background gradually blurs and dims while the fight artwork fades in from blur to clarity. A Canvas exactly matching the real `Kisara` title bounds uses a generated text alpha mask and the same diagonal fill frontier as the CSS tide. Within the already-red glyph area it draws a deep crimson lacquer base, broad drifting blooms, nonparallel curved material folds, a soft moving meniscus, and bounded shimmer motes. At 100%, those internal forms pull toward the existing `53% / 57%` title core and crossfade into the established enchant layer; no effect launches from the old lower-meter position. After the title-core prelude completes, continued downward input advances the existing gloss threshold and hands control to the automatic warning/release timeline. The warning directly desaturates and raises the contrast of the actual scene, closes a dark elliptical focus field onto the title center, and keeps its black/white core, four unequal tapered rays, broken shards, and sparks above that field. Its rotation follows one uninterrupted wall-clock timeline across build, flash, and collapse instead of restarting at eased phase boundaries; only a restrained pulse-local shake remains near the impact. It then collapses sharply back into the core and hands off to an automatic eight-second segmented release. The sequence dwells through field lock, deep collapse, terminal-crush hold, rupture, and aftermath rather than sweeping one linear progress curve. A foreground pink black hole forms at the warning center while the title and background remain depth-blurred: layered far/near accretion arcs, a feathered absorption core, broken photon arcs, lensing paths, and bounded orbit particles build pressure before an inversion flash. The background WebGL pass leaves the middle third at the original UV while the outer thirds receive visible pull, fold, torsion, refraction sheen, and shock displacement; narrow screens apply a lower field scale because the subject spans more of the side zones. The title remains independently refracted through rupture, dissolves into the blast instead of visibly springing back, and reconstructs from blur only during the final aftermath. Three slingshot particle waves, curved bipolar jets, expanding echo rings, residual ribbons, longer-lived fragments, slower gravity-driven rain, and afterglow replace the abrupt particle-only ending. The burst Canvas is a top-level gate layer and is explicitly excluded from the common blur selector, so the black hole remains sharp above the blurred title. The lower state text now uses four material phases before switching to continuously changing machine-glitch output. Document scrolling is released only after both stages settle at 100%; the next section and the return to the blue gate use a time-scaled damped spring rather than a distance-proportional jump.
...
- Superseded energy-beam reference: the previous local beam, front/back helix, exterior coils, U-turn flight, and wake-particle renderer remain in the file only as inactive rollback-era code behind `renderLegacyChargeBeam = false`. The lower meter is an absolute, clipped `1px` scheduler node with forced zero opacity and no layout contribution. Do not restore or extend this beam path unless the user explicitly asks to revisit it.
- Current charge prelude: the active `0-100%` presentation is owned by the title-infusion Canvas, not the lower meter. Its mask is rebuilt from the live Georgia title metrics on font readiness and resize, so desktop/mobile bounds match the DOM title exactly. The former `2.7s` flight intro is now a bounded `2.2s` internal convergence: material blooms and curved folds compress toward the title core, a masked radial handoff raises the existing enchant layer, and the infusion fades before the later blade gloss. Reversing after a completed release resets the Canvas opacity, fill, burst state, and title material; a same-tab second cycle was browser-validated.
...
1. Obtain user visual acceptance for the WebGL liquid-title takeover; tune idle softness and pointer-pressure deformation without restoring the rejected 2D overlay or fixed external lines.
2. Add character cutouts or puppet layers only after suitable source material is explicitly selected.
