...
- Implementation state: three independent frontends are locally available. The Kisara homepage interaction, article layout, and redesigned Blog/Works/Game/Me pages have passed the current local test/build/browser checks and are awaiting final user visual acceptance; they have not been pushed. Checkpoint `47331c7` preserves the redesigned home continuation, Blog poster archive, footer, and favicon. The current uncommitted homepage refinement replaces the blue/fight-only opening continuation with scene-aware sampling for base, five memory scenes, three transformation scenes, and fight art; it also makes the black/white warning warmup repeatable before each release cycle. The subject-aware lensing checkpoint is `1164fb4`, the latest warning rollback point is `d112f11`, the opening-transition rollback point is `cb1fa6e`, and the compressed-release checkpoint is `91356a2`.
...
- Memory, transformation, fight, reconstruction-wash, and memory-flare layers default to `visibility: hidden`; runtime visibility is enabled only while a layer contributes visible opacity. This limits simultaneous large filtered surfaces without changing the scene timeline.
