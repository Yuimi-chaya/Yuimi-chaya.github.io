...
- Implementation state: three independent frontends are locally available. The Kisara interaction and full-page presentation pass have passed the current local test/build/browser checks and are awaiting final visual acceptance; they have not been pushed. The subject-aware lensing checkpoint is `1164fb4`, the eight-second timing checkpoint is `f20ab43`, the latest warning rollback point is `d112f11`, the opening-transition rollback point is `cb1fa6e`, and the current compressed-release checkpoint is `91356a2`. The automatic release now completes in about 5.5 seconds, the pre-autoplay enchant sequence can reverse smoothly with the progress rail, the protected character core is redrawn from a sharp fight layer through a dual-feather mask while only the surrounding scene is distorted, and the reconstruction wash no longer flickers between two render loops.
