...
- Local state: the Kisara image transition, single-envelope charge beam, jet/coast/drift wake particles, post-gloss procedural warning, segmented foreground pink-black-hole release, side-only WebGL2 background lensing, independent title refraction/dissolve/reconstruction, multi-wave aftermath, lingering particle rain, persistent glitch status, and site-wide deep-indigo UI are implemented and browser-validated. The latest visual refinement is committed locally at `3b017b0` and is not pushed. Untracked user source materials remain separate and untouched (`102000325_p0.jpg`, `122472458_p0.png`, `gate-background.jpg`, `XP/`, `kisara/`, `showcase-output/`).
