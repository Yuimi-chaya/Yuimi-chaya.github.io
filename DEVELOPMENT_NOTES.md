...
- Status: image transition, continuous organic charge beam, two-stage overcharge sequence, persistent particle pools, segmented pink-black-hole release, side-field WebGL refraction, post-reconstruction eye/blade highlights, mouse parallax, and outward-eroding title data cells are implemented and browser-validated on desktop and `375x844` mobile viewports; publication is pending.
...
1. Obtain user visual acceptance for the sampled title-data erosion and stationary irregular black-hole well; tune cell reach, dark-layer overlap, or core size only from visual feedback.
