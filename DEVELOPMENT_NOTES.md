...
- Added a low-amplitude automatic scene-camera breath to every pre-fight image stage, including the initial blue artwork, all five memories, and frames `6/7/8`. Mixed low-frequency drift/scale with a very small faster tremor so settled wheel input no longer freezes the artwork; the envelope fades through reconstruction and is zero for the final fight state.
- Rollback point before the idle-motion refinement: `603abbc Integrate Kisara transformation scene sequence`. Browser runtime checks kept the lens at `ready`, showed no warning/error logs, measured idle camera movement across a one-second hold, sampled frame `8` from `10.5px / 1.000` to about `0.83px / 1.020`, and confirmed return-to-gate resets fight opacity to zero. `git diff --check`, `npm test` (3/3), and `npm run build` (36 pages, sitemap, Pagefind) passed.
