...
- Local state: the Kisara image transition, single-envelope charge beam, jet/coast/drift wake particles, post-gloss procedural warning, sharp foreground pink black hole, accretion/lensing layers, slingshot burst, lingering particle rain, persistent glitch status, and site-wide deep-indigo UI are implemented and browser-validated. The site-wide presentation pass is committed locally. The latest momentum-wake, single-RAF, screen-impact, and release-performance refinement remains local and is not pushed. Untracked user source materials remain separate and untouched (`102000325_p0.jpg`, `122472458_p0.png`, `gate-background.jpg`, `XP/`, `kisara/`, `showcase-output/`).
...
- Current energy refinement: the local beam uses front/back depth-separated helices, moving compression rings, helical fragments, and a motion-responsive nose shock. Wake particles inherit a low-pass-filtered beam-head velocity, then gain axial exhaust and lateral separation forces instead of appearing as two stationary side emitters. The energy renderer is guarded by one active RAF epoch; return-to-gate explicitly resets pose, particle, and beam-head clocks. Completed release work is scheduled with timeouts at the intended low frame rate instead of waking a no-op RAF every display frame.
- Current black-hole refinement: gravity now affects the full gate through differential background pull, chromatic separation, vignette, screen shake, ambient-particle attraction, and large lensing rings. The release uses two coherent major-axis particle waves with a smaller mobile/desktop budget, reduced orbit/rain limits, lower burst-canvas DPR, shorter particle lifetimes, swap removal, and fewer expensive path/highlight passes.
