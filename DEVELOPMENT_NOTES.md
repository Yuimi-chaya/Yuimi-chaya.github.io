...
- The wine-red tide now uses static patina, fine directional grain, and deeper tonal variation. Its wider alpha ramp and low-contrast violet diffusion band follow the actual spring-driven `--kisara-fill`, so the same transition works while advancing or rewinding without a separate animation timeline.
