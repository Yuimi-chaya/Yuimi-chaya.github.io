...
- Implementation state: three independent frontends are locally available. The Kisara interaction and full-page presentation pass have passed the current local test/build/browser checks and are awaiting final visual acceptance; they have not been pushed. The subject-aware lensing checkpoint is `1164fb4`, the eight-second timing checkpoint is `f20ab43`, the latest warning rollback point is `d112f11`, and the opening-transition rollback point is `cb1fa6e`. The homepage continuation now samples the actually visible base/fight artwork under the current `cover` crop into a short two-dimensional fade, clips that fade entirely inside the second section, keeps blue-state `SKIP` from flashing the fight artwork, and resets the hidden gate only after it leaves the viewport.
