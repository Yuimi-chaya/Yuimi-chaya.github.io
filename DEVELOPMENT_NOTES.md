...
- The clear blue source now has its own cold crystal/metal facets, fine grain, and tonal depth. The wine-red tide uses stronger old-lacquer patina, angular wear lines, directional scratches, and a textured vivid overlay instead of a flat color wash.
- The boundary is a broad red-violet-blue reaction band rather than a uniform highlight. Its grains, pits, and fracture lines stay clipped to the glyphs without blur; opacity rises slightly with scroll velocity and its position biases toward the current direction. Both values derive from the same spring-driven fill/velocity state, so advancing, pausing, and rewinding remain continuous.
