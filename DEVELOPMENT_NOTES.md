...
- Current black-hole refinement: the automatic release separates inward collapse, a particle-free full-screen vacuum shockwave, and the final detonation. Collapse now has a nonlinear terminal-crush phase: the event horizon and disk contract to roughly a fifth of their pre-collapse scale, flatten sharply, swallow orbit particles, pull denser streaks inward, and concentrate into a white-hot singularity before release. The local background lens is split into two independently masked image copies: a radial scale layer and an oppositely rotated/skewed tangential layer. Their moving annuli visibly displace wallpaper pixels without SVG-filter clipping, then fade fully during recovery. The event horizon is no longer a solid black fill; a deep blue-violet gradient, inward-moving depth rings, wisps, and finite-lifetime motes create a quiet bottomless interior. The wallpaper container itself remains fixed, and sustained viewport vibration is still avoided. Existing reduced DPR, bounded particles, swap removal, and low-frequency completed-state scheduling remain in place.
...
1. Obtain user visual acceptance for the latest terminal-crush, dual-band spatial-refraction, and layered abyss-core pass; tune collapse timing, lens radius, or void brightness only from new visual feedback.
