...
- The sealed wine-red area starts restrained and gains a separate numeric `--kisara-tide-charge` from the actual fill. Its recovered region progressively warms and reveals more energy texture across the whole filled area, while remaining visibly below the later unsealed vivid-red state.
- Removed the continuous multicolor reaction strip. The current boundary uses a dark depleted zone on the blue side plus sparse red fragments, interrupted dashes, and faint veins in a wide glyph-clipped recovery field. Pattern motion, opacity, and directional bias all derive from the same fill/velocity state, so advance and rewind share one continuous timeline.
