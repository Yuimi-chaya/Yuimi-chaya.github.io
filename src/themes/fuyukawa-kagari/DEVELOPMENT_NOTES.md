...
- Current Home hero scroll-gate follow-up: source confirms two premature-pass paths in the existing two-step reveal. `home-hero.mjs` released downward wheel after 120ms in docked state, while the identity card's transform/size transition takes 760ms and the avatar position up to 680ms; a wheel received after a small native scroll also marked the still-hidden card as "passed". The first correction used a near-top `scrollTo(0)` that could itself make the return/reopen path jump; superseded before publication after the user's follow-up report of repeated up/down jitter. The current version waits for the card's own transform `transitionend` with a 900ms fallback (immediate under reduced motion), consumes near-top wheel drift without snapping the document, and requires 36px of cumulative intentional upward wheel input to retract a docked card; downward input clears that reverse accumulator. A return/reopen fixture caught and corrected a premature fallback cancellation on tiny upward input. Genuine deep links and scroll below the hero are not captured. No artwork or transition styling is changed.
