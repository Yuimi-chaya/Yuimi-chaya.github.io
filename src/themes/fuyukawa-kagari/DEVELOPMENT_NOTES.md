...
- Current Home title follow-up starts from `542958255b21b91bee898af20e0bc56f1eef6d61`, clean Fuyukawa tree; unrelated root note, responsive-cover manifest and media remain dirty. User wants the entire left hero copy raised and "Yuimi Lab" rendered with Kisara Blog's baked-outline / pen-mask writing mechanism. Kisara stayed read-only; Fuyukawa's own `assets/hero-title.svg` has baked Georgia Italic outlines, 18 authored pen strokes and no runtime font dependency. Theme-local `tools/prepare-title-lettering.py` checks against an existing different asset instead of overwriting it. Offline 906px alpha comparison loses 252/54,335 solid pixels (0.46%) against the unmasked outline and adds none. Desktop copy moves up 52px while mobile keeps its existing 118/123px top offset. Title remains an external SVG to meet the tight HTML budget: generated Home 99,985/100,000 bytes, CSS 103.5/109.4 KiB. Fuyukawa tests 42/42 and direct Astro build 78 routes / 79 Pagefind pages pass; built asset and stylesheet resolve and the dev server returns HTTP 200 for the title SVG. Repository tests 319/320 pass, with the same unrelated 36-versus-24 responsive cover assertion. Performance numerical budgets for Fuyukawa pass, while the overall script still fails on the unrelated enlarged derivative set and reviewed source media copied to `dist`; do not alter those user materials for this theme work. No browser/subagents used; actual pen timing and hero placement remain for user acceptance. Logs: `%TEMP%/fuyukawa-title-{focused,theme-final,repository,build,budget}.log`; candidate raster `%TEMP%/fuyukawa-title-candidate-review.png` is offline, not a browser rendering.
