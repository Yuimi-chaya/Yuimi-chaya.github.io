...
| 1 | Request | `I` | legacy-derived metal links build on front/back canvases along two `I`-centered helical paths; no Canvas-frame loop remains | glass/residue only; the chain canvases clear on scene exit |
| 2 | Counterattack | `S` | directional pink/white/blue charge follows the curve and releases at impact | a static charged edge |
| 3 | Contract | first `A` | legacy enchant material and gloss feed a heart imprint centered inside the upper counter; its dark impression remains behind the glyph while the normally composited heart body, lock pulse, collapse, and shard burst remain visibly in front | a faint glyph-clipped residue only; the heart canvases clear after the action |
| 4 | Transformation | `R` | one local WebGL glyph continuously interpolates from an undistorted hard material into the soft liquid state; a separate clipped data pass contributes only cells and scan traces, never another complete glyph | the same WebGL `R` remains visible and liquid at settle without a layer swap |
...
- Non-active letters remain low-opacity Liquid Glass-style glyph lenses with empty interiors, one aligned restrained dark rim, subtle edge separation, and one narrow slow caustic sweep. They must not use a solid full-glyph text shadow, an offset duplicate highlight glyph, or a broad translucent fill. Past residue is limited to a local glyph-clipped diagonal trace; future letters remain clean glass. Scene-local canvases, enchant layers, gloss, cuts, charges, and fractures are hidden and cleared whenever their letter is not active.
...
- Transformation never swaps between a hard CSS `R` and a soft Canvas `R`. When WebGL is available, glass, material, enchant, and gloss are suppressed while one lens Canvas mixes the same source texture between undistorted and liquid samples through the shared action progress; moving progress in either direction therefore morphs the same visible glyph instead of hiding one layer to reveal another. The reconstruction Canvas draws only clipped cells and scan traces. The generic active-letter scale entrance is disabled, layout dimensions are transform-independent, font bounding metrics keep the shared baseline, and the renderer is destroyed on scene exit.
