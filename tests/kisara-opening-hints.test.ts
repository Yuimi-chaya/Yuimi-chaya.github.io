...
test("Home carries one indexed KISARA wordmark contract through all six scenes", () => {
  ...
  assert.match(stageStyles, /\.kisara-letter-contract-back \{ z-index: 2; \}/);
  assert.match(stageStyles, /\.kisara-letter-contract-front \{[\s\S]*?z-index: 12/);
  assert.match(stageStyles, /\.kisara-letter-contract-front \{[\s\S]*?mix-blend-mode: normal;/);
  assert.match(letterFxSource, /const centerY = height \* 0\.46;/);
  assert.match(letterFxSource, /front\.globalCompositeOperation = "source-over";/);
  assert.match(stageStyles, /\.kisara-letter-data-canvas,[\s\S]*?width: 100%;[\s\S]*?height: 100%/);
  assert.match(letterFxSource, /fontBoundingBoxAscent/);
  assert.match(letterFxSource, /fontBoundingBoxDescent/);
  assert.match(letterFxSource, /float morph=smoothstep\(\.08,\.92,clamp\(uProgress,0\.,1\.\)\);/);
  assert.match(letterFxSource, /vec4 base=mix\(hard,liquid,morph\);/);
  assert.match(letterFxSource, /const sourceOpacity = this\.liquid \? 0 : 0\.82;/);
  assert.match(letterFxSource, /const liquidOpacity = this\.liquid \? 0\.96 : 0;/);
  assert.doesNotMatch(letterFxSource, /fillRect\(0, 0, width, height\)/);
  assert.match(letterFxSource, /const resizeLetterCanvas = \(canvas: HTMLCanvasElement/);
  assert.match(letterFxSource, /const width = Math\.max\(1, canvas\.clientWidth\);/);
  assert.match(letterFxSource, /const height = Math\.max\(1, canvas\.clientHeight\);/);
  assert.equal(letterFxSource.match(/resizeLetterCanvas\(canvas, 1\.(?:15|2)\)/g)?.length, 2);
  assert.doesNotMatch(letterFxSource, /const size = resizeCanvas\(canvas, 1\.2\);/);
  assert.match(letterFxSource, /this\.liquid\?\.destroy\(\);[\s\S]*?this\.liquid = null;[\s\S]*?removeProperty\("--kisara-r-source-opacity"\)/);
  assert.match(letterFxSource, /--kisara-r-source-opacity/);
  assert.doesNotMatch(letterFxSource, /--kisara-r-(?:glass|material|enchant)-opacity/);
  assert.match(stageStyles, /data-wordmark-scene="transformation"\]\[data-wordmark-action="running"\][\s\S]*?kisara-stage-letter\[data-letter-state="active"\] \{\s*animation: none;/);
  assert.match(stageStyles, /data-wordmark-scene="transformation"[\s\S]*?kisara-stage-letter-material \{\s*animation: none;\s*opacity: 0;\s*transition: none;/);
  assert.match(stageStyles, /data-wordmark-scene="transformation"[\s\S]*?kisara-stage-letter-glass \{\s*opacity: 0;\s*transition: none;/);
  assert.match(stageStyles, /data-wordmark-scene="transformation"[\s\S]*?kisara-stage-letter-enchant \{[\s\S]*?--kisara-r-source-opacity/);
